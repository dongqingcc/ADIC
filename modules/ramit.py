...
class ShallowModule(nn.Module):
    def __init__(self, in_chans, out_chans, kernel_size=3, stride=1):
        ...
        print(f"in_chans: {in_chans}, type: {type(in_chans)}")
        print(f"out_chans: {out_chans}, type: {type(out_chans)}")
    ...
    def __init__(self, depth, dim, num_head, chsa_head_ratio, window_size=8, head_dim=None,
        ...
        for d in range(depth):
            self.blocks.add_module(f'block{d}', DRAMiTransformer(dim, num_head, chsa_head_ratio,
                                                                 window_size, 0 if d % 2 == 0 else shift, head_dim,
                                                                 qkv_bias,
...
class Bottleneck(nn.Module):
    def __init__(self, dim, num_stages, act_layer=nn.GELU, norm_layer=ReshapeLayerNorm, mv_ver=1, mv_act=nn.LeakyReLU,
                 exp_factor=1.2, expand_groups=4):
        ...
        if mv_ver == 1:
            ...
            self.mobivari = MobiVari2(self.cat_dim, 3, 1, act=mv_act, out_dim=dim, exp_factor=exp_factor,
                                      expand_groups=expand_groups)
        self.act = act_layer()
        self.norm = norm_layer(dim)
    ...
    def __init__(self, dim, kernel_size=3, stride=1, mv_ver=1, mv_act=nn.LeakyReLU, exp_factor=1.2, expand_groups=4):
        ...
        if mv_ver == 1:
            ...
            self.mobivari = MobiVari2(dim + dim // 4 + dim // 16 + dim, kernel_size, stride, act=mv_act, out_dim=dim,
                                      exp_factor=exp_factor, expand_groups=expand_groups)
...
class Reconstruction(nn.Module):
    def __init__(self, target_mode, dim, kernel_size=3, stride=1, num_mv=2, mv_ver=1, mv_act=nn.LeakyReLU,
                 exp_factor=1.2, expand_groups=4):
        ...
        for i in range(num_mv):
            if mv_ver == 1:
                ...
                self.mobivari.add_module(f'mobivari{i}',
                                         MobiVari2(dim, kernel_size, stride, mv_act, None, exp_factor, expand_groups))
    ...
    def flops(self, resolutions):
        ...
        flops += H * W * self.kernel_size * self.kernel_size * self.dim * self.out_chans * (
                    self.upscale ** 2)  # self.conv
...
class RAMiT(nn.Module):
    def __init__(self, in_chans=3, dim=64, depths=(6, 4, 4, 6), num_heads=(4, 4, 4, 4), head_dim=None,
                 ...
                 act_layer=nn.GELU, norm_layer=ReshapeLayerNorm, tail_mv=2, weight_init='', target_mode='light_x1',
                 img_norm=True,
        ...
        self.stage1 = EncoderStage(depths[0], dim, num_heads[0], chsa_head_ratio, window_size, head_dim, qkv_bias,
                                   mv_ver,
        ...
        self.stage2 = EncoderStage(depths[1], dim, num_heads[1], chsa_head_ratio, window_size, head_dim, qkv_bias,
                                   mv_ver,
                                   hidden_ratio, act_layer, norm_layer, attn_drop, proj_drop, drop_path, helper, mv_act)
        self.down2 = Downsizing(dim, dim, norm_layer, mv_ver, mv_act)
        self.stage3 = EncoderStage(depths[2], dim, num_heads[2], chsa_head_ratio, window_size, head_dim, qkv_bias,
                                   mv_ver,
                                   hidden_ratio, act_layer, norm_layer, attn_drop, proj_drop, drop_path, helper, mv_act)
        self.bottleneck = Bottleneck(dim, len(depths) - 1, act_layer, norm_layer, mv_ver, mv_act)
        self.stage4 = EncoderStage(depths[3], dim, num_heads[3], chsa_head_ratio, window_size, head_dim, qkv_bias,
                                   mv_ver,
                                   hidden_ratio, act_layer, norm_layer, attn_drop, proj_drop, drop_path, helper, mv_act)
